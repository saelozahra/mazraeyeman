...
'	Main.amaroid.TrackView(ActionBar.title&"  - جدید")
	...
	ActionBar.Menu.Add2(1,1,"تمام صفحه",X1.GetDrawable("baseline_fullscreen_white_24")).ShowAsAction=2
	
	If Stared Then
		ActionBar.Menu.Add2(2,2,"مورد علاقه",X1.GetDrawable("baseline_star_white_24")).ShowAsAction=2
	Else
		ActionBar.Menu.Add2(2,2,"مورد علاقه",X1.GetDrawable("baseline_star_border_white_24")).ShowAsAction=2
	End If
...
Sub ActionBar_MenuItemClick (Item As ACMenuItem)
	Select Item.Id
		Case 1
			fullscreen_video_act.videoUrlStr=VidUrl
			StartActivity(fullscreen_video_act)
		Case 2
			
			If Stared Then
				Stared = False
				ActionBar.Menu.FindItem(2).Icon=X1.GetDrawable("baseline_star_border_white_24")
				
				'TODO
'				Main.sql1.ExecNonQuery("UPDATE content set star = 0 WHERE id = " & library.selectedContent )
			Else
				Stared = True
				ActionBar.Menu.FindItem(2).Icon=X1.GetDrawable("baseline_star_white_24")
