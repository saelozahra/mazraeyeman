...
Sub Process_Globals
	...
	Dim EditMahsoolMap As Map = Null
End Sub
...
Sub Globals
	...
	Private zamanekasht_et,sennemahsool_et,zamanebazderakht_et,zamanekesht_et,fasele_kasht_et,radif_metr_et,chah_et,zamanefasleroshd_et,zamane_goldehi_et As EditText
	...
	Dim EditedProductTitle As String
	Dim EditedCategoryID As String
	Dim EditedPicUrl As String
...
'	Main.amaroid.TrackView(ActionBar.title)
	...
	zamanekesht_et.Typeface=SaeloZahra.font
	...
	tonaj_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	amount_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	zaman_eraa_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	bazarche_des_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	zamanebazderakht_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	zamane_goldehi_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	zamanefasleroshd_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	chah_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	radif_metr_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	fasele_kasht_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	zamanekesht_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	zamanekasht_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	sennemahsool_et.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	name_mahsool_lbl.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	gorooh_mahsool_spinner.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	gheshlagh_map_btn.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	yeylagh_map_btn.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	selectPicBtn.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	masahateYahteKeshET.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	
	Label3.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	Label5.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	Label6.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
	gorooheMahsoolLbl.Background=SaeloZahra.CD(SaeloZahra.ColorLight)
...
'	pdpd.Initialize("pdpd",1400,1300,SaeloZahra.font,SaeloZahra.colorLight,"ثبت","لغو","امروز")
	...
	pdpd.Initialize("pdpd",1402,1400,SaeloZahra.font,SaeloZahra.ColorDark,"ثبت","بازگشت","امروز")
	...
	Try
		If EditMahsoolMap.IsInitialized Then
			' If Edit Page
			ActionBar.Title = SaeloZahra.CSB("ویرایش محصولات")
			save_btn.Text="ویرایش محصول"
			
			vahed_spinner.SelectedIndex 	= vahed_spinner.IndexOf(EditMahsoolMap.Get("vahed"))
			noueKhakSpinner.SelectedIndex 	= noueKhakSpinner.IndexOf(EditMahsoolMap.Get("khak"))
			noueAbyari_Spinner.SelectedIndex= noueAbyari_Spinner.IndexOf(EditMahsoolMap.Get("abyari"))
				
			If EditMahsoolMap.Get("bazarche")=="yes" Then
				BazarcheSwitch_CheckedChange(True)
			Else
				BazarcheSwitch_CheckedChange(False)
			End If
			
			Dim EditMahsoolGroup 	= EditMahsoolMap.Get("group") As String
			masahateYahteKeshET.Text= EditMahsoolMap.Get("masahat")
			EditedPicUrl			= SaeloZahra.site_url&"reseller/fileupload/product/"&EditMahsoolMap.Get("pic")
			EditedCategoryID		= EditMahsoolMap.Get("category")
			EditedProductTitle		= EditMahsoolMap.Get("product_title")'EditMahsoolMap.Get("product")
			sennemahsool_et.Text 	= EditMahsoolMap.Get("sen_mahsol")
			chah_et.Text 			= EditMahsoolMap.Get("khoroji_chah")
			zamanekasht_et.Text 	= EditMahsoolMap.Get("zaman_kasht")
			zamanekesht_et.Text 	= EditMahsoolMap.Get("zaman_kesht")
			tonaj_et.Text 			= EditMahsoolMap.Get("vazn")
			zaman_eraa_et.Text 		= EditMahsoolMap.Get("zaman_eraa")
			amount_et.Text 			= EditMahsoolMap.Get("ghimat")
			bazarche_des_et.Text 	= EditMahsoolMap.Get("des")
			zamane_goldehi_et.Text 	= EditMahsoolMap.Get("goldehi_derakhtan")
			zamanebazderakht_et.Text= EditMahsoolMap.Get("tavarom_javane")
			zamanefasleroshd_et.Text= EditMahsoolMap.Get("fasele_roshd")
			radif_metr_et.Text 		= EditMahsoolMap.Get("fasele_radif")
			fasele_kasht_et.Text 	= EditMahsoolMap.Get("fasele_kesht")
			gheshlaghdate_et.Text	= EditMahsoolMap.Get("gheshlagh_time")
			yeylaghdate_et.Text		= EditMahsoolMap.Get("yeylagh_time")
			
			Dim ivimgdnld As HttpJob
			ivimgdnld.Initialize("ivimgdnld",Me)
			ivimgdnld.Download(EditedPicUrl)
			Wait For (ivimgdnld) jobDone(ivimgdnld As HttpJob)
			If ivimgdnld.Success Then
				ImageView1.Bitmap = ivimgdnld.GetBitmap
			End If
			
		End If
	Catch
		Log("edit mahsool error")
		Log(LastException)
	End Try
	
	
	jobListeGroup.Initialize("jobListeGroup",Me)
	If EditMahsoolGroup<>"" Then
		gorooh_mahsool_spinner.Tag = EditMahsoolGroup
	Else
...
Sub Activity_Resume
	...
	If File.Exists(SaeloZahra.dir,"gheshlagh_lat") Then
		...
		csbcheck.Color(SaeloZahra.ColorDark)
	...
	If File.Exists(SaeloZahra.dir,"yeylagh_lng") Then
		...
		csbcheck1.Color(SaeloZahra.ColorDark)
...
'				Dim errorCode As Int = root.Get("errorCode")
				If Job.GetString == "success" Then
					...
					ToastMessageShow(SaeloZahra.CSB("عملیات با موفقیت انجام شد"),True)
				End If
...
'					Dim category_id As String = colroot.Get("category_id")
				...
				If EditedProductTitle<>"" Then name_mahsool_spinner.SelectedIndex 	= name_mahsool_spinner.IndexOf(EditedProductTitle)
			...
			Case "jobListeGroup"
				...
				LogColor(SaeloZahra.json_url&"product_list.php?type=new&cat="&ThisGroupTypeID,Colors.LightGray)
				downloadListMahsoolJob.Download(SaeloZahra.json_url&"product_list.php?type=new&cat="&ThisGroupTypeID)
				
				If EditedCategoryID<>"" Then gorooh_mahsool_spinner.SelectedIndex 	= gorooh_mahsool_spinner.IndexOf(EditedCategoryID)
...
Sub zamanekesht_et_FocusChanged (HasFocus As Boolean)
	witchDatePicker = "zamanekesht_et"
	If HasFocus Then pdpd.show
End Sub
...
Sub pdpd_DateSelected(PersianYear As Int,PersianMonth As Int,PersianDay As Int)
	...
	Select witchDatePicker
		...
		Case "zamanekesht_et"
			zamanekesht_et.Text = fulldate
			zamanekesht_et.Tag  = YearMountDay
...
Sub save_btn_Click
	...
	If ThisProductID < 1 Or ThisGroupTypeID<1 Then
		name_mahsool_spinner.RequestFocus
		ToastMessageShow(SaeloZahra.CSB("نام محصول را وارد کنید"),False)
		
		Return
	End If
	
	Dim googleweightyeylag As String=select_map_act.yeylagh_lat
	Dim googleheightyeylag As String=select_map_act.yeylagh_lng
	...
	Dim googleweightgheshlagh As String=select_map_act.gheshlagh_lat
	Dim googleheightgheshlagh As String=select_map_act.gheshlagh_lng
	If File.Exists(SaeloZahra.dir,"gheshlagh_lat")Then
...
'		mavarede_darkhasti
		mapSubmit.Clear
		If EditMahsoolMap.IsInitialized Then
			mapSubmit.Put("id",		EditMahsoolMap.Get("id"))
		End If
		mapSubmit.Put("group",		gorooh_mahsool_spinner.Tag)
		mapSubmit.Put("category",	ThisGroupTypeID)
		mapSubmit.Put("product",	ThisProductID)
		mapSubmit.Put("zaman_kasht",zamanekesht_et.Text)
		mapSubmit.Put("zamane_kesht",zamanekesht_et.Text)
...
'		ImageView1.Bitmap = LoadBitmap(Dir,FileName)
		ImageView1.Bitmap = LoadBitmapSample(Dir, FileName,ImageView1.Width*1.2,ImageView1.Height*1.2)
...
'	CropImageInGallery(fd.Dir,fd.FileName)
	...
	Else
		BSFileChooser1.Show3("tFile", Array As String("jpeg", "jpg", "png"))
		ToastMessageShow("BS File Chooser :(",False)
	End If
...
Private Sub noueAbyari_Spinner_ItemClick (Position As Int, Value As Object)
	If Position = 2 Then
		chah_et.Enabled=False
		SaeloZahra.SetAlpha(chah_et,0.4)
	Else
		chah_et.Enabled=True
		SaeloZahra.SetAlpha(chah_et,1)
	End If
End Sub